...
const AthleteDetails = () => {
  ...
  const [showLogout, setShowLogout] = useState(false); // For toggling logout option
    ...
    <div>
      ...
      <nav className="bg-[#203C5C] sticky top-0 z-10 py-4 shadow-lg">
        <div className="flex justify-between items-center px-6 text-white">
          ...
          {/* Profile Picture and Logout Option */}
          <div
            className="relative flex items-center space-x-3 cursor-pointer"
            onClick={() => setShowLogout(!showLogout)} // Toggle logout visibility
          >
            ...
            {showLogout && (
              <div
                className="absolute top-14 right-0 bg-white rounded-md shadow-lg p-3"
                style={{
                  zIndex: 50,
                  display: "flex",
                  alignItems: "center",
